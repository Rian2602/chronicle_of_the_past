...
def test_dot_reapply_keeps_power_and_adds_duration():
    ...
    assert state.statuses["goblin"] == [StatusEffect(kind="poison", duration=5, power=3)]
...
def test_dot_duration_capped_at_max_status_duration():
    ...
    apply_status(state, "goblin", "poison", power=3, duration=2)
    apply_status(state, "goblin", "poison", power=3, duration=2)
    assert state.statuses["goblin"] == [StatusEffect(kind="poison", duration=3, power=3)]
    state2 = make_state(cap=3)
    apply_status(state2, "goblin", "burn", power=5, duration=99)
    assert state2.statuses["goblin"] == [StatusEffect(kind="burn", duration=3, power=5)]
...
def test_control_reapply_refreshes_duration_not_additive():
    ...
    assert state.statuses["goblin"] == [StatusEffect(kind="sleep", duration=1, power=0)]
...
def test_tick_deals_dot_damage_and_removes_expired():
    ...
    assert state.statuses["goblin"] == [StatusEffect(kind="burn", duration=1, power=5)]
...
def test_tick_player_hp_mutated_for_player_actor():
    ...
    assert state.statuses["player"] == [StatusEffect(kind="bleed", duration=1, power=4)]
