...
def test_ritual_ready_hanya_flag_roundtrip():
    """ritual_ready hanya di flags (P0.1) — round-trip tetap utuh."""
    state = _state()
    assert state.flags.get("ritual_ready") is None
    state.flags["ritual_ready"] = True
    data = state.to_dict()
    restored = GameState.from_dict(data)
    assert restored.flags.get("ritual_ready") is True


def test_save_lama_dengan_ritual_ready_backfill_ke_flag():
    """Save lama (field ritual_ready:True) → flag diset (P0.1)."""
    data = _state().to_dict()
    data["ritual_ready"] = True
    restored = GameState.from_dict(data)
    assert restored.flags.get("ritual_ready") is True


def test_save_lama_tanpa_ritual_ready_backfill():
    """Save lama tanpa ritual_ready — flag tetap None."""
    state = _state()
    data = state.to_dict()
    restored = GameState.from_dict(data)
    assert restored.flags.get("ritual_ready") is None
