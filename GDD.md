...
**Kurva insight:** tiap tingkatan kira-kira **2,5×** dari sebelumnya (rasio 3,0× → 2,7× → 2,5× → 2,5× → 2,4×). Angka final hidup di `data/cultivation/` — tabel di atas adalah referensi desain.
...
- **Risiko:** bisa **gagal** → cedera **sementara** (−25% stat selama 2 hari game) dan 30% peluang memicu pertarungan **inner demon**. **Tidak ada penalti permanen** (lihat §24.1). Sukses dasar 55%, +5% per poin stat pendukung jalur, +10–20% dari pil breakthrough, cap 90%.
...
- **Urutan giliran (order):** dihitung **sekali saat pertarungan dimulai** dari `agility` (tertinggi duluan), lalu tetap untuk seluruh pertarungan — bukan inisiatif dinamis per giliran (§24.1).
- **Tim 4 anggota:** protagonis + maksimal 3 rekan/binatang roh aktif (aturan rekrut: §20).
- **Defend / Escape / Observe / Item** — aksi standar; **Observe gratis** (tidak memakai giliran), Defend/Escape/Item memakai giliran (daftar perintah lengkap: §18).
...
- **Miss:** `max(5%, 20% − agility × 0.1%)` — minimum 5%.
...
| **Binatang Roh** | Rekan spiritual; **rekrut (kalahkan → ikat) ATAU menetas dari telur** — dua-duanya aktif (§24.1). Ikut bertarung, punya elemen & teknik sendiri, bisa berevolusi sekali. Detail: §20. |
...
> Nama di atas adalah **keputusan desain terkunci** (§24.1) — dapat diganti hanya bila ada alasan desain yang kuat dan disetujui pengguna.

---

## 11. Musuh & Bos

Tingkatan kolom "akhir arc" mengikuti §4.1 — **kurva bos diselaraskan dengan tingkatan pemain**: bos utama setiap arc berada satu tingkatan di bawah puncak pemain, kecuali bos final.
...
- **Kind requirement** (engine quest): `talk`, `enemy`, `map`, `flag`, `collect`, `kill_count`, `escort`, `breakthrough` (butuh pemain mencapai tingkatan tertentu).
- **Quest percabangan:** beberapa quest punya 2–3 penyelesaian yang memengaruhi reputasi dan flag ending (§21).
- **Flag kelulusan:** quest yang selesai **otomatis** men-set flag `quest<id>_done` oleh engine (§24.1) — event memakai flag ini sebagai trigger (§15).
...
## 15. Event Engine (fondasi gating & narasi)
...
| `observe` (alias `amati`) | Analisis musuh: lihat HP/qi/elemen/status — **gratis**, tidak memakai giliran (§6.1) |
...
- **Migrasi:** `schema_version` dinaikkan saat struktur berubah; engine punya `migrations: dict[versi_lama → fungsi]`. **Backfill wajib:** saat load, untuk setiap quest di `quests.done` yang belum punya flag `quest<id>_done`, set flag itu (mencegah event gate mati di save lama).
...
> **Catatan:** binatang roh memakai **dua-duanya** (rekrut + menetas) — bukan salah satu (§24.1).
...
### 21.3 Cara Melawan Entitas Kuno (Ending 1)
...
9. **Nama dunia final:** kerajaan **Ashenfeld**, desa awal **Emberfall** (`village_emberfall`), sekte **Awan Biru** (`sect_azure`); nama karakter di §10 adalah keputusan desain terkunci.
10. **Jumlah konten target:** 32 quest utama + 10 quest faksi (detail §22).
11. **Formula keseimbangan:** angka konkret di §6.4 & §17.
...
*Dokumen ini adalah sumber kebenaran desain. Perubahan yang bertentangan dengan dokumen ini wajib didiskusikan dengan pengguna (AGENTS.md §1).*
